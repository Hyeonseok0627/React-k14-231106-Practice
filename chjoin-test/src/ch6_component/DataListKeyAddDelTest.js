...
const DataListKeyAddDelTest = () => {
  ...
  // 기본 아이디 4가지를 사용했고, 그 이후의 id 값 5부터.
  const [nextId, setNextId] = useState(5);

  // 리스트 출력하기. -> 리스트 컴포넌트, 리스트의 아이템 컴포넌트
  // key 값 의무사항입니다. 하지만, 오류 먼저 확인 후 설정하기.
  // 아이템 요소를 출력하는 부분
  ...
  // text input 부분이 변경 시, 세터 함수로 변경사항을 업데이트를 함.
  ...
  const onClickText = () => {
    const nextTestArr = testArr.concat(
      ...
      {
        id: nextId,
        // inputText, 입력된 내용이 계속 변경을 감지하면서, 최종 단어가 업데이트가 됨.
        text: inputText,
      }
    );
  ...
  // filter 이용하기.
  // 특정한 데이터를 동작하게 하는 것이니 ()여기에 id를 넣어줌
  const onRemoveText = (id) => {
    // item.id !== id -> 틀리면 참, 같으면 거짓
    // item.id == id -> 같으면 참, 다르면 거짓
    // filter(콜백함수(조건)), filter 조건이 참이 되는 요소만 뽑아서, 새로운 배열을 만든다.(자주 사용함) => 그래서 삭제할 때, 많이 이용됨.
    // 예) 원본 id : 1~4, 삭제할 아이디가 3번
    // 1~4 검사
    // 1 !==3 : true -> 1: 필터됨, 결과 배열에 [1]
    // 2 !==3 : true -> 2: 필터됨, 결과 배열에 [1,2]
    // 3 !==3 : false -> 3: 필터 안됨, 결과 배열에 [1,2]
    // 4 !==3 : true -> 4: 필터됨, 결과 배열에 [1,2,4]
